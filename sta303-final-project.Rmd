...
# group 'Advance' and 'Active' line together, recode gender, and rescale income, age
cust <- full_cust %>% 
  mutate(y = ifelse(line == "Advance" | line == "Active", 1, 0),
         gender = factor(ifelse(is.na(sex) | sex == "Intersex", "Other", sex)),
...
table(cust$released_year)
cust %>% group_by(y) %>% summarize(mean(median_inc))
...
# Q1
prop.table(table(cust$gender, cust$y), 1)
...
```{r}
# Q1 models
mod1 <- lme4::glmer(y ~ age + med_inc_10k + gender + 
                      (1 + med_inc_10k + age | released_year), 
                    family = 'binomial', data = cust,
                    control=glmerControl(optimizer="bobyqa",  optCtrl=list(maxfun=2e5)))
summary(mod1)

mod2 <- lme4::glmer(y ~ age + med_inc_10k + gender + 
                      (1 + med_inc_10k | released_year), 
                    family = 'binomial', data = cust,
                    control=glmerControl(optimizer="bobyqa",  optCtrl=list(maxfun=2e5)))
summary(mod2)

mod3 <- lme4::glmer(y ~ age + med_inc_10k + gender + 
                      (1 + age | released_year), 
                    family = 'binomial', data = cust,
                    control=glmerControl(optimizer="bobyqa",  optCtrl=list(maxfun=2e5)))
summary(mod3)

mod4 <- lme4::glmer(y ~ age + med_inc_10k + gender + 
                      (1 | released_year), 
                    family = 'binomial', data = cust,
                    control=glmerControl(optimizer="bobyqa",  optCtrl=list(maxfun=2e5)))
summary(mod4)
